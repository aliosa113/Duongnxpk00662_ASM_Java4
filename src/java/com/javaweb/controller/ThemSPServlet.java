    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            while (i.hasNext()) {
                ...
                if (fi.isFormField()) {
                    if (fi.getFieldName().equalsIgnoreCase("tensanpham")) {
                        TenSP = fi.getString("UTF-8");
                    } else if (fi.getFieldName().equalsIgnoreCase("maloaisanpham")) {
                        MaLoaiSP = Integer.parseInt(fi.getString("UTF-8"));
                    } else if (fi.getFieldName().equalsIgnoreCase("giasanpham")) {
                        GiaSP = Integer.parseInt(fi.getString("UTF-8"));
                    } else if (fi.getFieldName().equalsIgnoreCase("ngaydang")) {
                        NgayDang = new SimpleDateFormat("yyyy-MM-dd").parse(fi.getString());
                    } else if (fi.getFieldName().equalsIgnoreCase("filename")) {
                        fileName = fi.getString();
                    } else if (fi.getFieldName().equalsIgnoreCase("mota")) {
                        MoTa = fi.getString();
                    }
        ...
        Sanpham sp = new Sanpham(TenSP, MaLoaiSP, GiaSP, NgayDang,fileName, MoTa);
...
//            out.println("<html>");
//            out.println("<head>");
//            out.println("<title>Servlet ThemSPServlet</title>");
//            out.println("</head>");
//            out.println("<body>");
//            out.println("<h1>Servlet ThemSPServlet at " + request.getContextPath() + "</h1>");
//            out.println("</body>");
//            out.println("</html>");
//        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(ThemSPServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
